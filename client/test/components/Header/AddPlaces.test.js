...
import AddPlaceFooter from '../../../src/components/Header/AddPlace';
...
describe('AddPlace', () => {
	...
	test('hridayab: should not display "This place has already been added" message when isPlaceAdded is false', () => {
		const props = {
			isPlaceAdded: false,
			addedPlaces: [],
			placeActions: {
		...
		const { queryByTestId } = render(<AddPlaceFooter {...props} />);
	
		const message = queryByTestId('exclamation-circle');
