...
export class Place {
	constructor(placeObj) {
		...
		this.country = placeObj.country;
        	...
        	this.defaultDisplayName = this.buildDefaultDisplayName(placeObj);
	}
    ...
    buildStreetAddress(placeObj){
        if ('streetAddress' in placeObj && placeObj.streetAddress) {
            return placeObj.streetAddress;
        } else {
            return this.hasHouseNumber(placeObj) + this.hasRoad(placeObj) + this.hasSuburb(placeObj);
        }
    }

    buildDefaultDisplayName(placeObj){
        if ('defaultDisplayName' in placeObj && placeObj.defaultDisplayName) {
            return placeObj.defaultDisplayName;
        } else {
